...
generate spaces network = case model network of
   ...
   Right layers -> unlines code
      ...
      where names      = assignNames $ groupBlocks layers
            ...
            --possibly generate the set_emb_weight method
            setEmb     = generateSetEmbWeights prefix names

            --all the code
            code       = ["\"\"\"",
                          "A Pytorch model generated by MetaTorch",
                          "\"\"\"",
...
generateForward prefix vars namedLayers = 
   let
      ...
      --get all the layers that need to be called in "forward"
      steps        = filter (callable . fst) namedLayers

      --generate the code to class each layer
...
genCall _ (UnSqueeze index) _            = printf "tensor = tensor.unsqueeze(%d)" index
   ...
   in
      unlines $ map (tab 1) 
               ["def fit(self, train_data, dev_data, num_epochs, learning_rate, reg):",
                ...
                tab 1 "optim = SGD(params, learning_rate, weight_decay=reg, momentum=0.9)",
                tab 1 "self.train()",
                tab 1 "epoch = 1",
                tab 1 "",
...
{- Generate the set embedding weights method -}
generateSetEmbWeights :: String -> LayerNames -> String
generateSetEmbWeights prefix layerNames =
   let
      tab ln = prefix ++ ln
      
      --create the code for the method
      code name = unlines $ map tab
         ["def set_emb_weights(self, pretrained):",
         tab "\"\"\"",
         tab "Set the embeddings layer's weights to a matrix of pretrained values",
         tab "\"\"\"",
         tab $ printf "self.%s.weight = nn.Parameter(pretrained)" name, 
         tab $ printf "self.%s.weight.requires_grad = False" name
         ]
   in
      --NOTE this assumes only one embedding layer is present
      --pick the first embedding layer if any exists, generate code for it
      maybe "" (code . snd) (find (isEmbedding . fst) layerNames)
...
layerType (UnSqueeze _)           = "unsqueeze"
...
isEmbedding :: Layer -> Bool
isEmbedding (Embedding _ _) = True
isEmbedding _               = False
